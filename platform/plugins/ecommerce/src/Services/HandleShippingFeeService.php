...
// echo ($shipping);die;
    $result = [];
    $has45CategoryId = false;
    $shippingFee = 0; // Initialize shippingFee to a default value
     ...
     if ($shipping) {
        ...
        // Calculate the total price for items in categories other than 45
        $totalOtherCategoryPrice = 0;
        $items = array_map(function ($itemId, $itemData) {
           // Find the product by item_id
         $product = Product::find($itemId);  // Assuming item_id corresponds to product_id in Product table
 
         // Retrieve the first category ID for the product (or handle multiple categories if needed)
         $categoryId = $product ? $product->categories->first()->id : null;
//         echo "<pre>";
// print_r($categoryId);die;
         // Calculate the total price by multiplying the price by quantity (qty)
         $totalPrice = $itemData['price'] * $itemData['qty'];  // Multiply price by quantity

            return [
                'item_id' => $itemId,
                'price' => $totalPrice,  // Use the total price (price * qty)
                'category_id' => $categoryId,  // Add category_id here
            ];
        }, array_keys($data['items']), $data['items']);
//print_r($itemData);die;
        $totalOtherCategoryPrice = 0;

        foreach ($items as $item) {
            // Check if 'item_id' exists in the item array
            if (isset($item['item_id'])) {
                // Add the item total price to totalOtherCategoryPrice if it's not in category 45
                if ($item['category_id'] !== 45) {
                    $totalOtherCategoryPrice += $item['price'];
                }

                if ($item['category_id'] === 45) {
                    $has45CategoryId = true;
                }
            } else {
                // Handle the case where 'item_id' is missing
                continue;
            }
        }

        // Determine the shipping fee based on the total price of items in other categories
        if ($totalOtherCategoryPrice < 500 && $totalOtherCategoryPrice !== 0) {
            $shippingFee = 49; // Apply shipping fee of 49 if the total price of other category items is less than 500
        }

        if ($rule) {
            $ruleDetail = $rule->items->where('city', $city)->where('is_enabled', 1)->first();
            if ($ruleDetail) {
                $result[$rule->id] = [
                    'name' => $rule->name,
                    'price' => $rule->price + $ruleDetail->adjustment_price + $shippingFee,
                ];
            } else {
                $result[$rule->id] = [
                    'name' => $rule->name,
                    'price' => $rule->price + $shippingFee,
                ];
            }
        } else {
            if ($has45CategoryId) {
                ...
                foreach ($rules as $rule) {
                    ...
                    if ($ruleItem) {
                        $result[$rule->id] = [
                            'name' => $rule->name,
                            'price' => max($rule->price + $ruleItem->adjustment_price + $shippingFee, 0),
                        ];
                    } else {
                        $result[$rule->id] = [
                            'name' => $rule->name,
    ...
    // If no rule was matched, create a default shipping option
    if (empty($result)) {
