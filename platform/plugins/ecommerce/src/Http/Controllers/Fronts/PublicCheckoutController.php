...
// print_r($data);die;
    ...
    public function postCheckout(
    	...
    	if (auth('customer')->check()) {
        $addresses = auth('customer')->user()->addresses;
     	$addressId = $request['address']['address_id'];
        }
      	
    	$zipCode = null;
    	if(!$zipCode){
    		if ($request->address['zip_code']) {
                $zipCode = $request->address['zip_code'];
        	}
    	}
    
    	// Loop through the addresses to find a match
    	if (!empty($addresses)) {
    		// Loop through the addresses to find a matching address_id
    		foreach ($addresses as $address) {
       			 if ($address->id == $addressId) {
           		 // If a match is found, set the zip_code
        ...
        foreach ($products as $product) {
            // Check if attributes exist and category_id is set
            if (isset($product['categories'])) {
                // Add category_id to the array
                $categoryIds[] = $product['categories'];
            }
        }

        $jsonCategoryIds = json_encode($categoryIds);
        $categoryIds = json_decode($jsonCategoryIds, true);
//print_r($categoryIds);die;
        // Variables to check
        $productName = "Ornamental Fishes";
        $productId = 45;

        // Initialize a variable to hold the result
        $productExists = false;

        // Loop through the array to check for the product
   ...
   // print_r($products);die;
        if ($productExists) {
    		if ($zipCode) {
        		 if (strpos($zipCode, '700') !== 0) {
                  // If zip code does not start with '700', show error
            		$productName = "Ornamental Fishes";
            		$errorMessage = "$productName category product is not available in your pin code.";
            		return $this
                		->httpResponse()
                		->setError()
   ...
   public function checkFish(Request $request)
{
    $products = Cart::instance('cart')->products();
        $categoryIds = [];
    	$addresses = [];
   ...
   // print_r($products);die;
    ...
    return response()->json(['success' => true, 'message' => 'Product is available.']);
}
}
