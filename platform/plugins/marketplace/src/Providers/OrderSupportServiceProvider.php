...
public function getShiprocketToken() {
    // Check if token exists and is still valid in cache
    ...
    if (!$token) {
        ...
        if (!$token) {
            return response()->json(['error' => 'Unable to generate Shiprocket token.'], 500);
        }

        // Store the token in cache for 24 hours
        Cache::put('shiprocket_token', $token, 1440);  // 1440 minutes = 24 hours
    }
...
$cityName = City::getCityNameById($request['address']['city']);
$countryName = Country::getCountryById($request['address']['country']);
$stateName = State::getStateById($request['address']['state']);
...
$orderData = [
'order_id' => $mpSessionData[0]['created_order_id']. $orderId,
...
'billing_last_name' => $request['address']['name'] ?? '',
'billing_address' => $request['address']['address'] ?? '',
'billing_address_2' => '',
'billing_isd_code' => '',
'billing_city' => $cityName ?? '', // Using the dynamic city name
'billing_pincode' => $request['address']['zip_code'] ?? '',
'billing_state' => $stateName ?? '', // Using the dynamic state name
'billing_country' => $countryName ?? '', // Using the dynamic country name
...
'shipping_customer_name' => $request['address']['name'] ?? '',
'shipping_last_name' => '',
'shipping_address' => $request['address']['address'] ?? '',
'shipping_address_2' => '',
'shipping_city' => $cityName ?? '', // Using the dynamic city name
'shipping_pincode' => $request['address']['zip_code'] ?? '',
'shipping_country' => $countryName ?? '', // Using the dynamic country name
'shipping_state' => $stateName ?? '', // Using the dynamic state name
'shipping_email' => $request['address']['email'] ?? '',
'shipping_phone' => $request['address']['phone'] ?? '',
'order_items' => [],
...
'length' => 20, // Valid number
'breadth' => 10,
'height' => 10,
'weight' => 0.5 ,// Use numeric values
'ewaybill_no' => '',
'customer_gstin' => '',
'invoice_number' => '',
...
$pickupLocations = [
[
'pickup_location' => 'LM STUDIO',
'address' => '4B S.R Das road',
'city' => 'Kolkata',
'state' => 'West Bengal',
'country' => 'India',
'pin_code' => '700026',
...
[
'pickup_location' => 'LM GALLERY',
'address' => '21/4 RUSSA ROAD 3RD LANE',
'city' => 'Kolkata',
'state' => 'West Bengal',
'country' => 'India',
'pin_code' => '700033',
...
// Process `items` and set order items dynamically
if (empty($request['pickup_location'])) {
$defaultPickupLocation = $pickupLocations[1]['pickup_location']; // Default to first location
$orderData['pickup_location'] = $defaultPickupLocation; // Set default pickup location
} else {
$orderData['pickup_location'] = $request['pickup_location'];
}

// Assuming $products is a collection or array of Product objects
// Check if $products is an array or collection and not null
// Ensure $products is valid and not null
// Ensure $products is valid and not null
...
// print_r($product);
// die;
        // Access SKU safely from the product
        $sku = 'DEFAULT_SKU_' . $product->id;// Use 'DEFAULT_SKU' if sku is not set
// echo "hi";
// echo $sku;
// echo "<br/><br/>";

        $units = max($product->qty ?? 1, 1); // Ensure units is at least 1
        $selling_price = $product->price ?? 100.0; // Default to 100 if selling price is not set

        // Ensure product name is always set
        $item_name = $product->name ?? 'Default Product Name'; // Fallback for missing product name

        // Check if product has 'values' (variations) and it's an array
        if (isset($product->values) && is_array($product->values)) {
            foreach ($product->values as $value) {
                // Access cartItem from the variation (if exists)
                $cartItem = $value->cartItem ?? null; // Safe access to cartItem

                // If there's a valid cartItem, use its qty; otherwise fallback to 1
                $units = $cartItem ? max($cartItem->qty ?? 1, 1) : max($product->qty ?? 1, 1);

                // Add order item data for each variation
                $orderData['order_items'][] = [
                    'item_id' => $product->id, // Use product's ID
                    'sku' => $sku, // SKU from product
                    'units' => $units, // Units (from cartItem or default to 1)
                    'selling_price' => $selling_price, // Selling price from product
                    'name' => $value->item_name ?? $item_name, // Use value's item_name or fallback
                ];
            }
        } else {
            // If no variations, add the simple product data
            $cartItem = $product->cartItem ?? null; // Access the cartItem for the simple product
            $units = $cartItem ? max($cartItem->qty ?? 1, 1) : max($product->qty ?? 1, 1);

            // Add order item for simple product
            $orderData['order_items'][] = [
                'item_id' => $product->id,
                'sku' => $sku,
                'units' => $units,
                'selling_price' => $selling_price,
                'name' => $item_name, // Use product name
            ];
        }
    }
} else {
    // Handle the case when $products is not an array or object
    echo "Error: No products data available.";
}
$orderData['payment_mode'] = 'Prepaid';
// // For debugging, print the order items
...
// die;
// Step 1: Generate the Shiprocket token
$tokenDD = $this->getShiprocketToken();
if (!$tokenDD) {
// dd('Failed to generate Shiprocket token.');
return response()->json(['error' => 'Unable to generate Shiprocket token.'], 500);
}

// Step 2: Create the order with Shiprocket
$data = Shiprocket::order($tokenDD)->create($orderData);
