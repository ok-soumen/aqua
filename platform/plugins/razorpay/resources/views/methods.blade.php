...
@if (get_payment_setting('status', RAZORPAY_PAYMENT_METHOD_NAME) == 1)
                ...
                $(document).off('click', '.payment-checkout-btn').on('click', '.payment-checkout-btn', function(event) {
                    ...
                    if (method === 'stripe' && $('.stripe-card-wrapper').length > 0) {
                        ...
                        Stripe.card.createToken(form, function(status, response) {
                         ...
                         $.ajax({
    url: '/ajax/checkout/check-fish', // Replace with your actual API endpoint
    type: 'POST', // Changed to POST
    data: new FormData($paymentCheckoutForm[0]), // Use FormData to collect form data
    ...
    success: function(response) {
        if (response.success) {
            callRazorPayScript(); // Call Razorpay if successful
            _self.removeAttr('disabled');
            _self.html(submitInitialText);
        } else {
            // Show an error message if payment is not available
            toastr.options = {
            ...
            toastr.error(response.message || 'Payment not available for this location.', "Error");
            _self.removeAttr('disabled');
            _self.html(submitInitialText);
        }
    },
    error: function(xhr) {
        // Handle error and show a toastr notification
        toastr.options = {
        ...
        toastr.error("An error occurred while checking product availability.", "Error");
        _self.removeAttr('disabled');
        _self.html(submitInitialText);
    }
});
